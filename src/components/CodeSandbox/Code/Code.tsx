...
export const Code = memo(function Code(props: CodeProps) {
  ...
  const selectedModel = openedModels?.find((model) => model.path === selectedModelPath)
  ...
  const handleSelectModel = (path: string) => {
    ...
    setOpenedModels((opened) => {
      ...
      return [...opened, models.find((model) => model.path === path)].filter(Boolean) as FileModel[]
    })
  }
  ...
  const previewMarkdown = useMemoizedFn(() => {
    ...
    // 检查是否已经存在预览 tab
    const existingPreview = openedModels.find((model) => model.path === previewPath)
    if (existingPreview) {
      // 如果已经存在，直接选择它
...
function getLangEditor(language: SupportedLanguage) {
  return function LangEditor(props: Omit<EditorProps, 'language' | 'defaultLanguage'>) {
    return <Editor {...props} language={language} defaultLanguage={language} />
  }
}
